...
void mysqlcontroller::initdb() {
    ...
    try {
		...
		string connstr = "tcp://" + string(_hostname) + ":";
		...
		DatabaseDetails<sql> dbaseobjs;
		dbaseobjs.Driver = get_driver_instance();
		dbaseobjs.ConnectionObject = dbaseobjs.Driver->connect(connstr,_username,_password);

		initdb(dbaseobjs);
		
		/* Create a connection */
		//driver = get_driver_instance();
		//con = driver->connect(connstr, _username, _password);
...
void mysqlcontroller::createTables() {
	...
	char* createTblStmt = new char[strlen(CREATESCHEMA) + strlen(_databasename) + 1];
	strcpy(createTblStmt, CREATESCHEMA);
	strcat(createTblStmt,_databasename);
	...
	stmt->execute(SQLHelper::getCreateTableSQL(SQLHelper::TableToCheck::Directories));
	con->commit();
