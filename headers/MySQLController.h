...
#define CHECKIFDIREXISTSSTMT "SELECT id FROM directories WHERE dirname = ? AND machinename = ?"
#define CHECKIFFILEEXISTSSTMT "SELECT id FROM files WHERE dirid = ? AND filename = ?"
#define UPDATEDIRINFOSTMT "UPDATE directories set dateverified=?, status='VERIFIED' WHERE id=?"
#define INSERTDIRINFOSTMT "INSERT INTO directories (dirname,dateadded,machinename) VALUES (?,?,? )"


class MySQLController : public virtual SQLFunctionsBase {
  public:
    MySQLController();
    MySQLController(char* host,char* dbase,char* uname,char* pwd,char* mname,int port = 3306);
